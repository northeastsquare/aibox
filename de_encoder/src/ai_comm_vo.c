...
AI_VO_MODE_E AI_GetVOMode(HI_U32 u32ChnNum)
{
    if(u32ChnNum == 1)
    {
        return AI_VO_MODE_1MUX;
    }
    else if(u32ChnNum <= 4)
    {
        return AI_VO_MODE_4MUX;
    }
    else if(u32ChnNum <= 8)
    {
        return AI_VO_MODE_8MUX;
    }
    else if(u32ChnNum <= 16)
    {
        return AI_VO_MODE_16MUX;
    }
    else
    {
        return AI_VO_MODE_16MUX;
...
HI_S32 AI_COMM_VPSS_Bind_VO(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, VO_LAYER VoLayer, VO_CHN VoChn)
{
    ...
    HI_S32 ret = HI_MPI_SYS_Bind(&stSrcChn, &stDestChn);
    if (ret != HI_SUCCESS)
    {
        printf("\033[0;31m vpss & vo bind failed %#x!\033[0;39m\n", ret);
        return ret;
    }
    else printf("Vo vpss bind successed\r\n");
    return HI_SUCCESS;
}
...
HI_S32 AI_COMM_VPSS_UnBind_VO(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, VO_LAYER VoLayer, VO_CHN VoChn)
{
    ...
    stDestChn.s32DevId = VoLayer;
    stDestChn.s32ChnId = VoChn;
    HI_S32 ret = HI_MPI_SYS_UnBind(&stSrcChn, &stDestChn);
    if (ret != HI_SUCCESS)
    {
        printf("\033[0;31m vpss & vo unbind failed %#x!\033[0;39m\n", ret);
        return ret;
    }
...
{
    VO_DEV                VoDev     = attr->Vodev;
    VO_LAYER              VoLayer   = attr->Vodev;
    AI_VO_MODE_E enVoMode  = AI_VO_MODE_1MUX;
    
    AI_COMM_VO_HdmiStop();
    AI_COMM_VO_StopChn(VoLayer, enVoMode);
    AI_COMM_VO_StopLayer(VoLayer);
    AI_COMM_VO_StopDev(VoDev);
...
HI_S32 AI_START_VO(AI_VO_SET_ATTR * attr)
{
    ...
    s32Ret = AI_COMM_VO_StartDev(VoDev, &stVoPubAttr);
    if(s32Ret != HI_SUCCESS)
    {
        printf("start VO fail for %#x!\n", s32Ret);
        return s32Ret;
    }
    printf("VO LOG 2\r\n");
    s32Ret = AI_COMM_VO_GetWH(stVoPubAttr.enIntfSync,
                                  &stLayerAttr.stDispRect.u32Width, &stLayerAttr.stDispRect.u32Height,
                                  &stLayerAttr.u32DispFrmRt);

    if (HI_SUCCESS != s32Ret)
    {
        printf("SAMPLE_COMM_VO_GetWH failed!\n");
        AI_COMM_VO_StopDev(VoDev);
        return s32Ret;
    }
    ...
    if (0 != memcmp(&attr->Disp_Rect, &stDefDispRect, sizeof(RECT_S)))
    {
    ...
    if (VO_PART_MODE_MULTI == enVoPartMode)
    {
        ...
        if (HI_SUCCESS != s32Ret)
        {
            printf("HI_MPI_VO_SetVideoLayerPartitionMode failed!\n");
            AI_COMM_VO_StopDev(VoDev);
            return s32Ret;
        }
    }

    const HI_U32 u32DisBufLen = 3;
    ...
    s32Ret = AI_COMM_VO_StartLayer(VoLayer, &stLayerAttr);
    if (HI_SUCCESS != s32Ret)
    {
        printf("SAMPLE_COMM_VO_Start video layer failed!\n");
        AI_COMM_VO_StopDev(VoDev);
        return s32Ret;
    }
    ...
    if(VO_INTF_MIPI == enVoIntfType)
    {
        ...
        if (HI_SUCCESS != s32Ret)
        {
            printf("HI_MPI_VO_GetVideoLayerCSC failed!\n");
            AI_COMM_VO_StopDev(VoDev);
            return s32Ret;
        }
        ...
        if (HI_SUCCESS != s32Ret)
        {
            printf("HI_MPI_VO_SetVideoLayerCSC failed!\n");
            AI_COMM_VO_StopDev(VoDev);
            return s32Ret;
        }
    }
    ...
    * start vo channels.
    ********************************/
    s32Ret = AI_COMM_VO_StartChn(VoLayer, enVoMode);
    if (HI_SUCCESS != s32Ret)
    {
        printf("SAMPLE_COMM_VO_StartChn failed!\n");
        AI_COMM_VO_StopLayer(VoLayer);
        AI_COMM_VO_StopDev(VoDev);
        return s32Ret;
    }
    ...
    ********************************/
    ...
    {
        AI_COMM_VO_HdmiStartByDyRg(enIntfSync, enDstDyRg);
    }
    ...
    ********************************/
    if(VO_INTF_MIPI == enVoIntfType)
    {
        AI_COMM_VO_StartMipiTx(enIntfSync);
    }
    return HI_SUCCESS;
...
HI_S32 AI_COMM_VO_StartChn(VO_LAYER VoLayer, AI_VO_MODE_E enMode)
{
    ...
    switch (enMode)
    {
        case AI_VO_MODE_1MUX:
            u32WndNum = 1;
            u32Square = 1;
            break;
        case AI_VO_MODE_2MUX:
            u32WndNum = 2;
            u32Square = 2;
            break;
        case AI_VO_MODE_4MUX:
            u32WndNum = 4;
            u32Square = 2;
            break;
        case AI_VO_MODE_8MUX:
            u32WndNum = 8;
            u32Square = 3;
            break;
        case AI_VO_MODE_9MUX:
            u32WndNum = 9;
            u32Square = 3;
            break;
        case AI_VO_MODE_16MUX:
            u32WndNum = 16;
            u32Square = 4;
            break;
        case AI_VO_MODE_25MUX:
            u32WndNum = 25;
            u32Square = 5;
            break;
        case AI_VO_MODE_36MUX:
            u32WndNum = 36;
            u32Square = 6;
            break;
        case AI_VO_MODE_49MUX:
            u32WndNum = 49;
            u32Square = 7;
            break;
        case AI_VO_MODE_64MUX:
            u32WndNum = 64;
            u32Square = 8;
            break;
        case AI_VO_MODE_2X4:
    ...
    {
        if( enMode == AI_VO_MODE_1MUX  ||
            enMode == AI_VO_MODE_2MUX  ||
            enMode == AI_VO_MODE_4MUX  ||
            enMode == AI_VO_MODE_8MUX  ||
            enMode == AI_VO_MODE_9MUX  ||
            enMode == AI_VO_MODE_16MUX ||
            enMode == AI_VO_MODE_25MUX ||
            enMode == AI_VO_MODE_36MUX ||
            enMode == AI_VO_MODE_49MUX ||
            enMode == AI_VO_MODE_64MUX )
        {
...
HI_S32 AI_COMM_VO_HdmiStartByDyRg(VO_INTF_SYNC_E enIntfSync, DYNAMIC_RANGE_E enDyRg)
{
    ...
    AI_COMM_VO_HdmiConvertSync(enIntfSync, &enVideoFmt);
    HI_S32 ret = HI_SUCCESS;
    ret = HI_MPI_HDMI_Init();
    if (ret != HI_SUCCESS)
    {
        printf("HDMI init failed %#x\r\n",ret);
        return ret;
    }
    ret = HI_MPI_HDMI_Open(enHdmiId);
    if (ret != HI_SUCCESS)
    {
        printf("HDMI open failed %#x\r\n",ret);
        return ret;
    }
    ret = HI_MPI_HDMI_GetAttr(enHdmiId, &stAttr);
    if (ret != HI_SUCCESS)
    {
        printf("HDMI attr got failed %#x\r\n",ret);
        return ret;
    }
    ...
    ret = HI_MPI_HDMI_SetAttr(enHdmiId, &stAttr);
    if (ret != HI_SUCCESS)
    {
        printf("HDMI attr set failed %#x\r\n",ret);
        return ret;
    }
    ret = HI_MPI_HDMI_Start(enHdmiId);
    if (ret != HI_SUCCESS)
    {
        printf("HDMI start failed %#x\r\n",ret);
        return ret;
    }
...
static HI_VOID AI_PRIVATE_VO_InitScreen1080x1920(HI_S32 fd)
{
    HI_S32 s32Ret;
    cmd_info_t cmd_info;
...
*/
static HI_VOID AI_PRIVATE_VO_InitScreen720x1280(HI_S32 s32fd)
{
    HI_S32     fd     = s32fd;
    HI_S32     s32Ret;
    HI_U8      cmd[30];
...
*/
static void AI_PRIVATE_VO_InitMipiTxScreen(VO_INTF_SYNC_E enVoIntfSync, HI_S32 fd)
{
    if( VO_OUTPUT_1080x1920_60 == enVoIntfSync ||
        VO_OUTPUT_1080P60      == enVoIntfSync)
    {
        printf("%s,%d,Init 1080p screen.\n",__FUNCTION__,__LINE__);
        // init screen for 1080x1920_60.
        AI_PRIVATE_VO_InitScreen1080x1920(fd);
    }
    else if (VO_OUTPUT_720x1280_60 == enVoIntfSync ||
             VO_OUTPUT_720P60      == enVoIntfSync)
    {
        printf("%s,%d,Init 720p screen.\n",__FUNCTION__,__LINE__);
        // init screen for 720x1280_60.
        AI_PRIVATE_VO_InitScreen720x1280(fd);
    }
    else
    {
        printf("%s,%d,Init 1080p screen.\n",__FUNCTION__,__LINE__);
        // init screen for 1080x1920_60.
        AI_PRIVATE_VO_InitScreen1080x1920(fd);
        printf("%s,%d,There is no screen to init\n",__FUNCTION__,__LINE__);
    }
}
#else
/*
...
*/
static void AI_PRIVATE_VO_InitMipiTxDev(VO_INTF_SYNC_E enVoIntfSync)
{
    printf("error: mipi_tx dev has not been initialized yet\n");
}
#endif
...
void AI_COMM_VO_StartMipiTx(VO_INTF_SYNC_E enVoIntfSync)
{
    ...
    */
    #if SAMPLE_VO_USE_DEFAULT_MIPI_TX
    AI_PRIVATE_VO_InitMipiTxScreen(enVoIntfSync,fd);
    #else
    AI_PRIVATE_VO_InitMipiTxDev(enVoIntfSync);
    #endif
