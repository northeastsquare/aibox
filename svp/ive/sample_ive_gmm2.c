...
HI_VOID SAMPLE_IVE_Gmm2_Uninit(SAMPLE_IVE_GMM2_S *pstGmm2)
{
    ...
    if (pstGmm2->pFpSrc != NULL)
        IVE_CLOSE_FILE(pstGmm2->pFpSrc);
    if (pstGmm2->pFpFg != NULL)
        IVE_CLOSE_FILE(pstGmm2->pFpFg);
    if (pstGmm2->pFpBg != NULL)
            ...
            HI_CHAR *pchFgFileName,HI_CHAR *pchBgFileName)
{
   ...
   SAMPLE_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret, GMM2_INIT_FAIL,
    ...
    memset(pstGmm2->stDilateCtrl.au8Mask, 255, 25);
    memset(pstGmm2->stErodeCtrl.au8Mask, 255, 25);
    pstGmm2->stCclCtrl.u16InitAreaThr     = 16;
    pstGmm2->stCclCtrl.u16Step             = 4;
    pstGmm2->stCclCtrl.enMode             = IVE_CCL_MODE_8C;
...
HI_S32 SAMPLE_IVE_GenFgMask(IVE_SRC_IMAGE_S *pstFg,IVE_DST_IMAGE_S *pstFgMask)
{
    HI_S32 s32Ret;
...
*/
HI_VOID SAMPLE_IVE_ReduFactor(IVE_IMAGE_S *pstFactor)
{
    HI_U16 i,j;
    HI_U8 *pu8Tmp;
...
*/
HI_BOOL SAMPLE_IVE_ChgFactorByDiffFrm(IVE_SRC_IMAGE_S *pstSrc,IVE_SRC_IMAGE_S *pstLastImg,
...
*/
HI_S32 SAMPLE_IVE_ChgFactorByGrad(IVE_SRC_IMAGE_S *pstFgMask,
...
*Adjustment factor
*/
HI_VOID SAMPLE_IVE_AdjustmentFactor(IVE_SRC_IMAGE_S *pstFactor,
...
HI_S32 SAMPLE_IVE_Image_Gmm2Proc(SAMPLE_IVE_GMM2_S *pstGmm2, VIDEO_FRAME_INFO_S* pstExtFrmInfo)
{
    ...
    pstGmm2->u32FrmNum ++;
     //1.Get Y
    //bInstant = HI_FALSE;
    // s32Ret = SAMPLE_COMM_IVE_DmaImage(pstExtFrmInfo,&pstGmm2->astSrc[pstGmm2->s32CurIdx],bInstant);
    // SAMPLE_CHECK_EXPR_RET(HI_SUCCESS != s32Ret,s32Ret,"Error(%#x),SAMPLE_COMM_IVE_DmaImage failed!\n",s32Ret);
    //s32Ret = SAMPLE_COMM_IVE_ReadFile(&(pstGmm2->astSrc[pstGmm2->s32CurIdx]), pstGmm2->pFpSrc);
    //SAMPLE_CHECK_EXPR_RET(HI_SUCCESS != s32Ret,s32Ret,"Error(%#x),Read src file failed!\n",s32Ret);
    ...
    if (1 == pstGmm2->stGmm2Ctrl.u8ModelNum)
    {
        //If the parameter u8ModelNum is set to 1, the parameter u16FreqReduFactor
        //is usually set to a small value at the first N frames. Here, N = 500.
        pstGmm2->stGmm2Ctrl.u16FreqReduFactor = (pstGmm2->u32FrmNum >= 500) ? 0xFFA0 : 0xFC00;
    }
    else
    {
        //If the parameter u8ModelNum is more than 1, the global life mode should be used at the first N frames,
        //and the parameter u16GlbLifeUpdateFactor is usually set to a big value. Here, N = 500.
        if(pstGmm2->u32FrmNum >= 500)
        {
            pstGmm2->stGmm2Ctrl.enLifeUpdateFactorMode = IVE_GMM2_LIFE_UPDATE_FACTOR_MODE_PIX;
        }
        else
        {
            pstGmm2->stGmm2Ctrl.u16GlbLifeUpdateFactor = 0xFFFF / pstGmm2->u32FrmNum;
        }
    }
    //printf("start ive_gmm2\n");
    //GMM2
    s32Ret = HI_MPI_IVE_GMM2(&IveHandle,&pstGmm2->astSrc[pstGmm2->s32CurIdx],&pstGmm2->stFactor,&pstGmm2->stFg,&pstGmm2->stBg,
        &pstGmm2->stMatchModelInfo,&pstGmm2->stModel,&pstGmm2->stGmm2Ctrl,bInstant);
    SAMPLE_CHECK_EXPR_RET(HI_SUCCESS != s32Ret,s32Ret,"Error(%#x),HI_MPI_IVE_GMM2 failed!\n",s32Ret);
    //printf("gmm2 finish %d\n", pstGmm2->u32FrmNum);
    //factor adjustment
    if (pstGmm2->u32FrmNum > 1)
    {
        s32Ret = SAMPLE_IVE_GenFgMask(&(pstGmm2->stFg),&(pstGmm2->stFgMask));
        SAMPLE_CHECK_EXPR_RET(HI_SUCCESS != s32Ret,s32Ret,"Error(%#x),SAMPLE_IVE_GenFgMask failed!\n",s32Ret);

        SAMPLE_IVE_AdjustmentFactor(&(pstGmm2->stFactor), &(pstGmm2->stFgMask),
                                &(pstGmm2->astSrc[pstGmm2->s32CurIdx]),&(pstGmm2->astSrc[1 - pstGmm2->s32CurIdx]),
                                &(pstGmm2->stDiffImg), &(pstGmm2->stMagImg), &(pstGmm2->stCurNormMag),
                                &(pstGmm2->stLastNormMag), &(pstGmm2->stBlob));
    ...
    if(pstGmm2->u32FrmNum > 2){
        //4.Dilate
        s32Ret = HI_MPI_IVE_Dilate(&IveHandle, &pstGmm2->stFg, &pstGmm2->stImg1, &pstGmm2->stDilateCtrl, bInstant);
        SAMPLE_CHECK_EXPR_RET(HI_SUCCESS != s32Ret,s32Ret,"Error(%#x),HI_MPI_IVE_Dilate failed!\n",s32Ret);

        //5.Erode
        s32Ret = HI_MPI_IVE_Erode(&IveHandle, &pstGmm2->stImg1, &pstGmm2->stImg2, &pstGmm2->stErodeCtrl, bInstant);
        SAMPLE_CHECK_EXPR_RET(HI_SUCCESS != s32Ret,s32Ret,"Error(%#x),HI_MPI_IVE_Erode failed!\n",s32Ret);

        //6.CCL
        bInstant = HI_TRUE;
        s32Ret = HI_MPI_IVE_CCL(&IveHandle, &pstGmm2->stImg2, &pstGmm2->stBlob, &pstGmm2->stCclCtrl, bInstant);
        SAMPLE_CHECK_EXPR_RET(HI_SUCCESS != s32Ret,s32Ret,"Error(%#x),HI_MPI_IVE_CCL failed!\n",s32Ret);
        ...
        //printf("blobtorect\n");
        IVE_CCBLOB_S* pstBlob = (IVE_CCBLOB_S*)pstGmm2->stBlob.u64VirAddr;
        SAMPLE_COMM_IVE_BlobToRect(pstBlob, &(pstGmm2->stRegion), IVE_RECT_NUM, 8,
            pstExtFrmInfo->stVFrame.u32Width, pstExtFrmInfo->stVFrame.u32Height,
            pstExtFrmInfo->stVFrame.u32Width, pstExtFrmInfo->stVFrame.u32Height);
        //printf("finish blob2rect\n");
    }
    //change cur image to last image
    pstGmm2->s32CurIdx = 1 - pstGmm2->s32CurIdx;
