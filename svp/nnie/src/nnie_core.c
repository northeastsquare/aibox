                            ...
                            SAMPLE_SVP_NNIE_PARAM_S *pstNnieParam, SAMPLE_SVP_NNIE_INPUT_DATA_INDEX_S *pstInputDataIdx)
{
    ...
    } else
    {
        ...
        u32Stride = pstNnieParam->astSegData[u32SegIdx].astSrc[u32NodeIdx].u32Stride;
        pu8PicAddr = (HI_U8 *) (pstNnieParam->astSegData[u32SegIdx].astSrc[u32NodeIdx].u64VirAddr);
        // printf("fill data h,w,c,stride:%d, %d, %d, %d, u32VarSize:%d\n", u32Height, u32Width, u32Chn, u32Stride, u32VarSize);
        ...
        {
            // printf("yvu420sp fill data bhwc: %d,%d,%d,%d\n", pstNnieParam->astSegData[u32SegIdx].astSrc[u32NodeIdx].u32Num, u32Height, u32Width, u32Chn);
            for (n = 0; n < pstNnieParam->astSegData[u32SegIdx].astSrc[u32NodeIdx].u32Num; n++)
            {
                for (i = 0; i < u32Chn * u32Height / 2; i++)
                {
                    ...
                    //memset(pu8PicAddr, 0, u32Width * u32VarSize);
                    // double sum = 0;
                    // for (int k = 0; k < u32Width * u32VarSize; k++){
                    //     sum += data[k];
                    // }
                    // printf("sum:%f",sum);
            ...
            //printf("yvu420sp fill data done\n");
    ...
    for (u32SegIdx = 0; u32SegIdx < u32SegNum; u32SegIdx++)
    {
        for (u32NodeIdx = 0; u32NodeIdx < pstNnieParam->pstModel->astSeg[u32SegIdx].u16DstNum; u32NodeIdx++)
        {
            ...
            } else
            {
                ...
                float *tmp = t.data;
                //printf("u32NodeIdx:%d",u32NodeIdx);
                //printf("nnieresult n:%d,ch:%d,h:%d,w:%d", t.n, t.channel, t.height, t.width);
                ...
                for (n = 0; n < pstNnieParam->astSegData[u32SegIdx].astDst[u32NodeIdx].u32Num; n++)
                {
                    for (i = 0; i < u32Chn; i++)
                    {
                        for (j = 0; j < u32Height; j++)
                        {
                            ...
                            {
                                // if (u32Width==7){
                                //     printf("network outputint:%d", *(ps32ResultAddr + k));
                                // }
