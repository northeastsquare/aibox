...
public:
    ...
    void run(unsigned char *input_img, nnie::clsRes &clsInfo);
    void run_logistic(cv::Mat input_img, std::vector<nnie::LogisticRes> &lres);
    void run_logistic(cv::Mat input_img, std::vector<nnie::LogisticRes> &lres, const double thresh);
    void run_logistic(unsigned char * pimg, std::vector<nnie::LogisticRes> &lres, std::vector<float> threshs);
private:
